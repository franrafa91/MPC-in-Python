...
class Train_and_Rail(SystemInterface):
    """
    Implementation of a train-rail system with a falling box
    
    Attributes:
        m float: Mass of the train
        g float: Gravity Coefficient
        theta float: Inclination angle for rail
        mu float: Friction Coefficient between train and rail
    """

    def __init__(self,  m=100, g=9.81, theta=math.pi/6, mu=1e-3):
        """
        Initialize the spring-damper system with a given set of parameters.

        Args:
            m float: Mass of the train (m)
            g float: damping coefficient (N·s/m)
            theta float: Inclination angle for rail
            mu float: Friction coefficient between train and rail
        """
        self.m = m
    ...
    def __call__(self, t, w, f= lambda t,w: 0):
        ...
        m, g, theta, mu = self.m, self.g, self.theta, self.mu
        ...
        dw = np.array([dx_box, dy_box, dvy_box, dx_train, dvx_train, dy_train])
        self.dw_sol.append(dw)

        return dw

    def plot_animation(self, t_sol, w_sol, controller, SKIPFRAMES=2):
